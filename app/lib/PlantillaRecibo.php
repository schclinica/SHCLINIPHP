...
class PlantillaRecibo extends FPDF{
// Cabecera de página
use Session;
use QrCode;
private $Recibo;
...
{ 
        ...
        $this->Cell(210, 0, $this->Image(isset(self::BusinesData()[0]->logo) ? "public/asset/empresa/" . self::BusinesData()[0]->logo : "public/asset/img/lgo_clinica_default.jpg", 6.5, null, 60, 28, 'jpg'), 0, 1, "C");

        $this->Ln(2);
        $this->SetFont("Courier", "B", 9);
        $this->SetX(0);

        $this->MultiCell(70, 2.9, isset(self::BusinesData()[0]->nombre_empresa) ?  "Clinica - " . utf8__(self::BusinesData()[0]->nombre_empresa) : 'xxxxxxxxxxxxxxxxxxxxxxxxxx', 0, "C");

        $this->Ln(3);
        $this->SetFont("Courier", "B", 9);
        $this->SetX(0);
        $this->Cell(74, 2, isset(self::BusinesData()[0]->ruc)  ? " RUC: " . self::BusinesData()[0]->ruc : " RUC: xxxxxxxxxxx", 0, 1, "C");
        ...
        $this->SetX(34);
        $this->setFont("Courier", "", 8);
        $this->Cell(10, 3, $this->getRecibo()[0]->fecha_recibo, 0, 1);

        $this->Ln(2);
        $this->SetX(4);
        $this->setFont("Courier", "B", 8);
        $this->Cell(3, 3, utf8__("N° de venta: "), 0, 0, "L");
        $this->SetX(25);
        $this->setFont("Courier", "", 7);
        $this->Cell(10, 3, $this->getRecibo()[0]->numero_recibo, 0, 1);

        $this->setFont("Courier", "B", 8);
        $this->Ln(3);
        $this->SetX(0);
        $this->MultiCell(74,2,utf8__($this->getRecibo()[0]->apellidos." ".$this->getRecibo()[0]->nombres),0,"C");
...
function Footer()
{
    ...
    $this->SetY(-40);
         ...
         $this->SetFont("Courier","B",7);
            ...
            /// codigo qr de prueba
             self::GenerateQr((isset(self::BusinesData()[0]->ruc)?self::BusinesData()[0]->ruc:'xxxxxxxxxx')."|".$this->getRecibo()[0]->numero_recibo."|".number_format($this->getIgv(),2,","," ")."|".number_format($this->getTotal(),2,","," ")."|".$this->getRecibo()[0]->fecha_recibo."|".
